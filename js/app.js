function abucket(n){var s=0;for(var i=0;i<n.length;i++){s+=n.charCodeAt(i);}return 'b'+('0'+(s%16)).slice(-2);}
...
-----END PUBLIC KEY-----`;
  ...
  function openWordBankAll() {
    ...
    function enSrcOf(w) { if (w.enAudio) { return w.enAudio.indexOf('?') === -1 && w.enAudio.indexOf('audio/') === 0 ? w.enAudio + '?v=' + ASSET_V : w.enAudio; } const _fn = String(w.en).toLowerCase() + '_en.mp3'; return 'audio/words/' + abucket(_fn) + '/' + _fn + '?v=' + ASSET_V; }
    function zhSrcOf(w) { if (w.zhAudio) { return w.zhAudio.indexOf('?') === -1 && w.zhAudio.indexOf('audio/') === 0 ? w.zhAudio + '?v=' + ASSET_V : w.zhAudio; } const _fn = String(w.en).toLowerCase() + '_zh.mp3'; return 'audio/words/' + abucket(_fn) + '/' + _fn + '?v=' + ASSET_V; }
