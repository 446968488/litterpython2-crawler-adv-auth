...
vm.runInContext(fs.readFileSync(path.join(root, 'js/figures_adv.js'), 'utf8'), sandbox, { filename: 'figures_adv.js' });
...
for (const ch of data.chapters) {
  if (ch.type === 'exam') {
    // 考试课：无 lessons，直接校验其 exercises
    total++;
    if (!ch.title) problems.push(`[${ch.id}] 缺标题`);
    if (!ch.markdown || !ch.markdown.trim()) problems.push(`[${ch.id}] 讲义(markdown)为空`);
    if (!ch.takeaway || !ch.takeaway.trim()) problems.push(`[${ch.id}] 缺 takeaway`);
    (ch.exercises || []).forEach((ex, i) => checkEx(ex, `[${ch.id}] 题${i + 1}(${ex.type})`));
    continue;
  }
  ...
  for (const les of ch.lessons) {
    ...
    (les.exercises || []).forEach((ex, i) => checkEx(ex, `[${les.id}] 题${i + 1}(${ex.type})`));
    // 代码沙盒
